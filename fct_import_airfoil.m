function [af] = fct_import_airfoil(filename)
%IMPORT_AIRFOIL import data from .dat file and convert it to a matrix with
%x and y coordinates (from the trailing edge to the trailing edge, upper
%surface first and then lower surface)
...
while ~ af_defined
    if (airfoil(1,1) > 1) && (airfoil(1,2) > 1) %if range defined
        ...
        if af_up(1,:) == af_down(1,:)
            af_down(1,:) = [];
        end
        af = cat (1, flip(af_up,1) , af_down); %one array output
