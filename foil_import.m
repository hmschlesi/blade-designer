...
%%import foils with given polar data

listing=dir('foils_4_import_pol');
l=length(listing);
for i=3:l
    file_folder=listing(i).folder;
    file_name=listing(i).name;
    name=erase(file_name, [".txt",".dat","_pol.dat"])
    file_path=join(['foils_4_importpol\',file_name]);
